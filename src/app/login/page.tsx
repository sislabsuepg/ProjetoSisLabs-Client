...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const data = await apiOnline.post('/login', { login: form.login, senha: form.senha });
      toast.success('teste!');
  ...
  const ErrorMessage = ({ field }: { field: string }) =>
    errors[field] ? <div style={{ color: 'red', marginTop: 4 }}>{errors[field]}</div> : null;
    ...
    <section id="login" className={`${styles.sectionLogin}`}>
      <div className={`${styles.sideLogo}`}>
        ...
        <p className="text-[1.2rem]">
          A instituição que, diferentemente de uma ruptura com o passado, <strong>avança</strong>{' '}
          a partir de suas <strong>conquistas</strong>.
        </p>
      </div>
      <div className={`${styles.sideLogin}`}>
        <div className="flex flex-col items-center justify-center">
          <PersonIcon sx={{ fontSize: 50 }} className="text-lightBlue" />
          <p className="text-light-blue">Acesso ao controle dos laboratórios do DEINFO</p>
        </div>
