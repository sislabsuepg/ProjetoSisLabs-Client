  ...
  return (
    <div className="w-full h-full flex flex-col justify-start">
        ...
        {(eventos.length === 0 && recados.length === 0) && (
          <p className="text-theme-gray font-normal">Nenhum evento ou aviso disponível.</p>
        )}
        {eventos.length > 0 && (
          <div className="mb-6">
            <h2 className="font-semibold text-lg text-theme-darkBlue mb-2">Eventos</h2>
            <ul>
              {eventos.map(evento => (
                <li key={evento.id} className="text-theme-gray font-normal">
                  <div className="bg-blue-100 rounded-md p-4 shadow-sm">
                    {evento.nome} - {new Date(evento.data).toISOString().split('T')[0].replace(/-/g, '/')} às {new Date(evento.data).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </li>
              ))}
            </ul>
        ...
        {recados.length > 0 && (
          <div>
            <h2 className="font-semibold text-[1.2rem] text-theme-blue mb-2 mt-5">Avisos</h2>
            <ul>
              {recados.map(recado => (
                <li key={recado.id} className="text-theme-gray font-normal">
                    <div className="bg-blue-100 rounded-md p-4 shadow-sm">
                    {recado.texto}
                    </div>
                </li>
